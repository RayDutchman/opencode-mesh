...
(() => {
  ...
  const state = { manifest: null, pc: null, channel: null, ready: null, pending: new Map(), streams: new Map(), sockets: new Map(), closed: false };
  ...
  async function connectP2P() {
    const manifestResponse = await nativeFetch('/_mesh/transport-manifest', { credentials: 'same-origin' });
    ...
    const answerResponse = await nativeFetch(state.manifest.p2p.offer, { method: 'POST', credentials: 'same-origin', headers: {'content-type':'application/json'}, body: JSON.stringify({ type: pc.localDescription.type, sdp: pc.localDescription.sdp }) });
   ...
   async function send(message) {
     ...
     // DataChannel 的 bufferedAmount 没有背压时，大文件会直接把通道推入关闭状态。
  ...
  class MeshWebSocket {
    ...
    constructor(input, protocols) {
      ...
      Promise.resolve(state.ready).then(() => {
        ...
        send({type:'ws_open', id:this.id, path:url.pathname, query:url.search.slice(1), headers:{}}).catch(error => this.fail(error));
      });
    }
  ...
  state.ready = connectP2P().catch(() => null);
  window.__ocmTransport = state;
  window.fetch = async (input, init) => {
    const url = new URL(typeof input === 'string' || input instanceof URL ? input : input.url, location.href);
    if (url.origin !== location.origin || url.pathname.startsWith('/_mesh/')) return nativeFetch(input, init);
  ...
  window.WebSocket = class extends MeshWebSocket {
    constructor(input, protocols) {
      const url = new URL(input, location.href);
      if (url.host !== location.host || !state.channel || state.channel.readyState !== 'open') {
