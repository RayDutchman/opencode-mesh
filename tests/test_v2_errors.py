...
def test_mesh_errors_have_json_content_type(monkeypatch, kind):
    """Errors generated by Mesh itself must be parseable as JSON by the SDK."""
    def handle(request):
        if kind == 'connection':
...
def test_p2p_stream_error_boundary(after_headers, reason):
    """Errors before the first frame return a JSON Response; errors after it abort the original response stream."""
