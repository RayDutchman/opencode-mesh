...
- 浏览器测试依赖安装在 `/tmp/mesh-check/browser-check`，不进入产品运行依赖。
...
- 模型验收遇到独立的供应商限制：免费模型返回限流、Device B 的 GLM 返回账户余额不足；这些是已成功发送后原样显示的模型错误，不是 UnsupportedContentType。继续用现有可用模型验证完整回复。
...
| 场景 | 实际结果 |
| --- | --- |
| Device A P2P 消息和事件流 | GPT-6 Astra / low 回复本次唯一标记 `MESH_dda48c553e22` |
| Device A Relay 消息和事件流 | 禁用 RTC 后回复本次唯一标记 `MESH_783b2f59940c` |
| Device A P2P / Relay 终端 | 创建、connect-token、连接及键盘输入均通过；printf 和 hostname 返回预期标记及 Device A |
| Device B P2P / Relay 终端 | 相同操作通过，hostname 返回 Device B |
| Device B 消息 | 两种传输均送达，Big Pickle 返回 Rate limit exceeded；尚未取得完整助手回复 |
...
| 原生多 Server 切换 | 首页 Device A → Device B，显示 Device B 会话列表；打开已有 Device B 会话 URL，返回首页再切回 Device A；未出现跨设备 404 |
| 自动测试 | 112 项通过，包含真实 Node URL/Request/Abort/通道行为及 Agent 请求字节透传 |

本地浏览器日志：`/tmp/mesh-check/v020-final-switch.log`、`v020-final-relay.log`、`v020-final-device-b.log`、`v020-final-device-b-p2p.log`。日志属于本地诊断资料，不打包公开。

用户验收剩余项：手机开启 v2rayNG 时的真实网络打洞情况；Device B 使用可用模型后的完整回复。当前环境不能替代这两项验收。独立代码审阅未完成，原因是 reviewer 模型限流；不能将自审描述为独立审阅通过。
