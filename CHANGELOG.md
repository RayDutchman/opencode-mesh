...
- 修复 Agent 关闭上游 WebSocket 时，缺省或空关闭原因导致关闭帧序列化异常的问题；显式 null 统一视为空原因。
- 修复 V2 懒加载预加载资源丢失来源设备的问题；前端资源命名空间升级，避免继续使用旧缓存。
- 网络变化或页面恢复时及时重试尚未建立的 P2P，后台协商不再给每个 Relay 请求增加等待。
- 统一设备在线判据；离线页明确目标设备，支持手动切换，状态查询失败时显示未知并继续恢复轮询。
- 页面从后台恢复时验证旧 P2P 通道，探测期间新请求走 Relay；3 秒未收到匹配心跳则重建，已发送操作不自动重放。
- Shell 脚本提示与代码注释统一为英语。
